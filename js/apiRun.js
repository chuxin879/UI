...
$(function(){
	...
	var url = lemon.config.global.admin+"/suite/listAll?projectId="+ projectId;
	$.ajax({
		url:url,
	...
	// (url,{"projectId":$("[name='projectId']").val()},function(ret){
	// 	if(ret!=null){
	// 		var suites = "";
		...
		var dialog = jqueryAlert({
		    ...
		    'buttons' :{
		        ...
		        '确定':function(){
		        	...
		        	var name = $('.pcAlert').last().find("[name='caseName']").val();
	...
	$(".btn-send").click(function(){
		...
		$.ajax({
			url:lemon.config.global.admin + "/api/run",
			headers:{
			...
			success:function(ret){
				if(ret.status=="1"){
					...
					$("[name='responseBody']").html("<pre>"+ret.data.body+"</pre>");
				}else{
					alert(ret.message);
				}
			}
